...
// Load the generated 2° Medio JSON as our base template
const templatePath = 'C:/Users/56940/.gemini/antigravity/brain/f241f1d5-1366-49fe-b98c-b98ca0270324/Didakta_Planificacion_Nueva_2Medio.json';
if (!fs.existsSync(templatePath)) {
  console.error('Template JSON not found at:', templatePath);
  process.exit(1);
}
const templatePlanning = JSON.parse(fs.readFileSync(templatePath, 'utf8'));
...
async function validateAll() {
  console.log('=== STARTING AUTOMATED EXPORTS & SUMMARIES VALIDATION ===');
  ...
  for (const grade of grades) {
    ...
    // B. Clone plan from template and customize for this grade/OA
    console.log(`[${grade}] Customizing planning content template...`);
    
    // Deep clone the template content
    const planningContent = JSON.parse(JSON.stringify(templatePlanning.content));
    
    // Customize title and objectives in template to match the new grade/OA
    planningContent.backward_design.objective = `${oa.codigo_oa}: ${oa.texto_oa}`;
    
    // Clear summaries to force the summarizer call
    delete planningContent.lirmi_summary;
    delete planningContent.utp_documentation;

    // C. Call /api/planner/summarize
    ...
    // D. Assemble complete planning object
    const planning = {
      ...
      reading_level: {
        estimated_level: templatePlanning.reading_level.estimated_level.replace('2° Medio', grade),
        warning_alert: templatePlanning.reading_level.warning_alert
      },
      content: {
        ...planningContent,
    ...
    // E. Export to PDF and Word for all 3 views
